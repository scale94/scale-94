...
const OUTPUT_PATH = path.join(ROOT, 'src', 'terminal', 'data', 'articles.academic.js');
...
function run() {
  ...
  if (!DRY_RUN && !fs.existsSync(CHUNKS_DIR)) {
  ...
  for (const file of files) {
    ...
    if (!DRY_RUN) {
      ...
      // Cache hit: skip renderMarkdown if hash matches + chunk exists
      const isCacheHit = !FORCE && cache[cacheKey] === rawHash && fs.existsSync(chunkPath);
  ...
  // ── Serialise lean index with loadContent() ──────────────────────────────
  const entries = articles.map(a => {
    ...
    const importPath = `./academic_chunks/${cname}.js`;
  ...
  console.log(`\n  ✓ Generated ${articles.length} academic chunk(s) → academic_chunks/`);
  console.log(`  ✓ Generated articles.academic.js (lean index, ${articles.length} entries).`);
  console.log('');
}
