...
import _somaArticles       from './data/articles.soma';       // static soma kernel entries (hand-curated)
import _miscArticles       from './data/articles.misc';       // static misc/fiction entries
import _generatedArticles  from './data/articles.generated';  // generated by import-kernel.js (have loadContent)
import autoArticles        from './data/loadArticles';        // auto-loaded .md stubs from content/soma_kernel
import tagIndex            from './data/tags.generated';      // tag → [{id,title}] associative index
import systemArticles      from './data/articles.system';     // pre-rendered system pages (manifesto, thesis, etc.)
import _academicArticles   from './data/articles.academic';   // generated by import-academic.js (cs2 PHD)
...
//   1. staticArticles  — hand-curated soma + misc entries (source of truth for IDs)
//   2. _generatedArticles — output of import-kernel.js; each entry carries loadContent
//   3. autoArticles    — Vite glob .md stubs; fallback when no generated entry exists
// Title-based dedup is intentionally removed — see prior comment.
const staticIds    = new Set(staticArticles.map(a => a.id));
const generatedIds = new Set(_generatedArticles.map(a => a.id));
const articles  = [
  ...staticArticles,
  ..._generatedArticles.filter(a => !staticIds.has(a.id)),
  ...autoArticles.filter(a => !staticIds.has(a.id) && !generatedIds.has(a.id)),
  // Academic PHD corpus — IDs are PHD-prefixed so collision risk is zero
  ..._academicArticles,
];
...
const App = () => {
  ...
  const handleKernelClick = useCallback((kernel) => {
    ...
    setTimeout(async () => {
      ...
      if (kernel.articleId) {
        // Priority: auto .md stub → generated (both have loadContent) → static fallback.
        const foundArticle =
          autoArticles.find(a => a.id === kernel.articleId) ??
          _generatedArticles.find(a => a.id === kernel.articleId) ??
  ...
  const handleCommand = (e) => {
    ...
    if (e.key === 'Enter') {
      ...
      if (['home', 'kernel', 'system'].includes(action)) {
        ...
        const q       = norm(query);
        const target  = articles.find(a => norm(a.id).includes(q) || norm(a.title).includes(q));
        const wasmEntry = target
          ? (wasmRegistry[target.id] ?? Object.values(wasmRegistry).find(e => norm(e.id).includes(q)))
          : null;
        ...
        if (!target) {
          ...
          setSystemLogs(prev => [
            ...prev,
            { time: now, msg: `  RUN_FAIL :: "${query}" — no executable found in registry.` },
            { time: now, msg: `  WASM registry: ${Object.keys(wasmRegistry).length} module(s) registered.` },
            { time: now, msg: `  Try: load ${query.toLowerCase().split(' ')[0]} | list | help` },
          ].slice(-2000));
        } else if (wasmEntry) {
          // ── WASM execution path ────────────────────────────────────────────
          appendSystemLog({ time: now, msg: `COMMAND: ${rawCmd}` });
          ...
          (async () => {
            try {
              ...
              const result = mod[wasmEntry.struct][wasmEntry.boot]();
              const lines  = result.split('\n');
              setSystemLogs(prev => [
                ...prev,
                { time: now, msg: `  ── KERNEL OUTPUT ─────────────────────────` },
                ...lines.map(l => ({ time: now, msg: `  ${l}` })),
                { time: now, msg: `  ──────────────────────────────────────────` },
              ].slice(-2000));
            } catch (err) {
              setSystemLogs(prev => [
      ...
      <main ref={mainRef} className="flex-grow overflow-y-auto overflow-x-hidden p-4 md:p-8 relative z-10 scroll-smooth" style={{ scrollPaddingTop: '100px' }}>
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          {tagCloudView && !selectedArticle && (
            <TagCloudView handleReturnToRoot={handleReturnToRoot} />
          )}
        </div>
        </Suspense>
      </main>
